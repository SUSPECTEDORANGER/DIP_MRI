...
def nii_to_image(niifile):
    ...
    slice_trans = []
    result = []
    arr_3d = np.random.rand(3480, 135, 189)
    arr_3d_index = 0
    for f in filenames:
        #开始读取nii文件
        ...
        fname = f.replace('.nii', '') 
        img_f_path = os.path.join(imgfile, fname+'-slice_enhance_contrast')
        # 创建nii对应图像的文件夹
        ...
        for i in range(85,z-60,1):   
            ...
            # result.append(slice)
            imageio.imwrite(os.path.join(img_f_path, '{}.png'.format(i)), slice_enhance_contrast)
